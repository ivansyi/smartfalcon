...
func IOStatsForProc() interface{} {
	...
	for device, _ := range diskStatsMap {
		...
		item := map[string]interface{}{
			"device":       device,
			"r_merge":      fmt.Sprintf("%d", IODelta(device, IOReadMerged)),
			"w_merge":      fmt.Sprintf("%d", IODelta(device, IOWriteMerged)),
			"rio":          fmt.Sprintf("%d", rio),
			"wio":          fmt.Sprintf("%d", wio),
			"r_sec":        fmt.Sprintf("%.2f", float64(delta_rsec)/2.0),
			"w_sec":        fmt.Sprintf("%.2f", float64(delta_wsec)/2.0),
			"avgrq_sz":     fmt.Sprintf("%.2f", avgrq_sz),                                             // avgrq-sz: delta(rsect+wsect)/delta(rio+wio)
			"avgqu_sz":     fmt.Sprintf("%.2f", float64(IODelta(device, IOMsecWeightedTotal))/1000.0), // avgqu-sz: delta(aveq)/s/1000
			"await":        fmt.Sprintf("%.2f", await),                                                // await: delta(ruse+wuse)/delta(rio+wio)
			"svctm":        fmt.Sprintf("%.2f", svctm),                                                // svctm: delta(use)/delta(rio+wio)
			"util_percent": fmt.Sprintf("%.2f", float64(use)/10.0),                                    // %util: delta(use)/s/1000 * 100%
		}
		L[device] = item
		//L = append(L, item)
	}
